    ...
    initialize(c) {
        ...
        if (c.el && c.el.onLoad) {
            ...
            c.el.onLoad = function() {
                ...
                that.get('toLoad').forEach(M => {
                    M.onLoad();
                });

                that.loadOnStart();
                that.initUndoManager();

                that.on('change:selectedComponent', that.componentSelected, that);
                that.on('change:changesCount', that.updateBeforeUnload, that);
            }
        } else {
            // Load modules
            deps.forEach(function(name) {
